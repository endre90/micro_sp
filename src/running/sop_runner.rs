...
// Semi NEW, problem -> it hangs sometimes where the operation cant finish.
...
//             SOPState::Initial => {
//                 let mut sop_enabled = state
//                     .get_bool_or_default_to_false(&format!("{}_sop_enabled", sp_id), &log_target);
//                 if sop_enabled {
...
//                 let mut stack_json =
//                     state.get_string_or_value(&format!("{}_sop_stack", sp_id), "[]".to_string(), &log_target);
...
//                 state.get_string_or_default_to_unknown(&format!("{}_information", operation.name), &log_target);
...
// NEW, works with emulator, but doesn't register trigger down and state to succ from robot
pub async fn sop_runner(
    sp_id: &str,
    model: &Model,
    connection_manager: &Arc<ConnectionManager>,
