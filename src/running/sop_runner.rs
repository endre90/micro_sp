...
pub async fn sop_runner(
    ...
    //         .sops
    //         .iter()
    //         .flat_map(|sop| sop.sop.get_all_operation_names())
    //         .collect::<Vec<String>>(),
    // );
        ...
        match redis_get_full_state(&mut con).await {
            Some(current_state) => {
                *last_known_state.write().await = Some(current_state.clone());
                let current_sop_id = current_state
                    .get_string_or_default_to_unknown(log_target, &format!("{}_sop_id", sp_id));
                ...
                let new_state = process_sop_tick(sp_id, model, &current_state, &log_target)?;
...
fn process_sop_tick(
    ...
    let mut sop_overall_state =
        state.get_string_or_default_to_unknown(&log_target, &format!("{}_sop_state", sp_id));
...
fn handle_sop_initial(
    ...
    if state.get_bool_or_default_to_false(&log_target, &format!("{}_sop_enabled", sp_id)) {
...
fn handle_sop_executing(
    ...
    let sop_id = state.get_string_or_default_to_unknown(&log_target, &format!("{}_sop_id", sp_id));
    let stack_json =
...
fn is_sop_failed(sp_id: &str, sop: &SOP, state: &State, log_target: &str) -> bool {
    match sop {
        SOP::Operation(operation) => {
            let op_state_str = state.get_string_or_default_to_unknown(&log_target, &operation.name);
