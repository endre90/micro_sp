...
pub fn parser(name: &str) -> () { //PlanningProblem {
    let mut f = File::open(&format!("src/models/gripper/instances/{}.pddl", name)).unwrap();
    ...
    let mut init: Vec<&str> = vec!();
    let mut goal: Vec<&str> = vec!();

    for p in parts {
        println!("{:?}", p);
        if p.starts_with("init") {
            let inits: Vec<&str> = p.split("init (").collect();
            init = inits[1].split(")\n          (").collect();
        }
        println!("pppppp: {:?}", init);
    }
