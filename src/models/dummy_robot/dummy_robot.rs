...
pub fn raar_model() -> PlanningProblem {
    ...
    let t1 = Transition::new(
        "start_activate",
        &Predicate::AND(vec![not_act_active.clone(), not_act_ref_active.clone(), not_any_measured_dummy.clone()]),
        &Predicate::AND(vec![ref_active.clone(), act_ref_active.clone()]),
    );

    let t2 = Transition::new(
        "finish_activate",
        &Predicate::AND(vec![not_act_active.clone(), act_ref_active.clone(), not_any_measured_dummy.clone()]),
        &Predicate::AND(vec![act_active.clone(), act_ref_active.clone()]),
    );

    let t3 = Transition::new(
        "start_deactivate",
        &Predicate::AND(vec![not_act_idle.clone(), not_act_ref_idle.clone(), not_any_measured_dummy.clone()]),
        &Predicate::AND(vec![ref_idle.clone(), act_ref_idle.clone()]),
    );

    let t4 = Transition::new(
        "finish_deactivate",
        &Predicate::AND(vec![not_act_active.clone(), act_ref_idle.clone(), not_any_measured_dummy.clone()]),
    ...
    let t5 = Transition::new(
        ...
        &Predicate::AND(vec![
            ...
            act_active.clone(),
            act_ref_active.clone(), not_any_measured_dummy.clone()
        ]),
