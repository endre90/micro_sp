...
pub async fn tf_interface(
    ...
    let keys = tf_interface_keys(sp_id);
    ...
    loop {
        ...
        tf_tick(sp_id, &mut con, &state, &log_target).await;
    }
}

/// The keys the TF interface reads and writes.
///
...
pub fn tf_interface_keys(sp_id: &str) -> Vec<String> {
    vec![
...
/// One tick of the TF interface: serve the pending request, if there is one.
///
/// The caller has already established from its snapshot that
/// `{sp_id}_tf_request_trigger` is set. Unlike the other tick bodies this one
/// writes its own answer, because the transform commands it dispatches touch the
/// transform keyspace rather than the state and have to be ordered against the
/// reply. It returns nothing for the caller to publish.
///
/// [`tf_interface`] calls it after its own trigger poll; the sequential runner
/// calls it when the shared snapshot shows the trigger set.
pub async fn tf_tick(sp_id: &str, con: &mut SPConnection, state: &State, log_target: &str) {
    ...
    if request_trigger {
        ...
        if request_state == ServiceRequestState::Initial.to_string() {
            ...
            match command.as_str() {
                "lookup" => {
                    match TransformsManager::lookup_transform(con, &parent, &child).await {
                        Ok(tf) => {
                            tf_lookup_result = tf;
                ...
                "reparent" => {
                    match TransformsManager::reparent_transform(con, &parent, &child).await
                    {
                ...
                "snap_to_parent" => {
                    match TransformsManager::snap_to_parent_transform(con, &parent, &child)
                        .await
                    {
                        Ok(()) => request_state = ServiceRequestState::Succeeded.to_string(),
                        Err(e) => {
                ...
                "insert" => {
                    match TransformsManager::insert_transforms(con, &tf_insert_transforms)
                        .await
                    {
                        Ok(()) => request_state = ServiceRequestState::Succeeded.to_string(),
                        Err(e) => {
