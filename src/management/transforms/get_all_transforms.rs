...
pub(super) async fn get_all_transforms(
    ...
    let scan_options = redis::ScanOptions::default()
        .with_pattern(format!("{}*", TF_PREFIX))
        .with_count(TF_SCAN_COUNT);
    let iter: redis::AsyncIter<String> = con_clone.scan_options(scan_options).await?;
